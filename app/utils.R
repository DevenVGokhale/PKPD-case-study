...
# Function to add observation noise
add_obs_noise <- function(df, prop_sd_cp = 0.126, add_sd_cp = 2.09, 
                         prop_sd_cyt = 0.2, add_sd_cyt = 0.5) {
  df |>
    mutate(
      # Add proportional + additive error to CP
      DV_CP = CP * (1 + rnorm(n(), 0, prop_sd_cp)) + rnorm(n(), 0, add_sd_cp),
      # Add proportional + additive error to CYT  
